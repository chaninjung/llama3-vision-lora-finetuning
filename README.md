...
4. (Optional) Extract only the LoRA adapter state\_dict:
...
* **r (Rank)**: 저차원 임베딩 차원. 값이 크면 품질 개선 가능하지만, 메모리/속도 트레이드오프 발생.
* **α (Alpha)**: Scaling factor; 16\~32 범위가 무난.
* **Dropout**: 과적합 방지를 위해 0.05 정도 설정.

<details>
<summary>🇺🇸 English Explanation</summary>

LoRA (PEFT) fine-tuning updates only a small subset of low-rank parameters rather than the full model, saving memory and training time. You specify hyperparameters like `r`, `alpha`, `dropout`, and `target_modules`.

```python
lora_config = LoraConfig(
    task_type=TaskType.CAUSAL_LM,
    inference_mode=False,
    r=args.lora_rank,            # default: 8
    lora_alpha=args.lora_alpha,  # default: 32
    lora_dropout=args.lora_dropout,  # default: 0.05
    target_modules=args.target_modules.split(","),  # ["q_proj","k_proj","v_proj","o_proj"]
    bias="none",
)
```

* **r (Rank)**: Low-rank embedding dimension for LoRA. Larger values can improve performance but cost more memory/compute.
* **α (Alpha)**: Scaling factor; 16–32 is a common choice.
* **Dropout**: Set around 0.05 to prevent overfitting.
...
단, 드물게 512 토큰을 넘는 예시가 있다면 뒤쪽이 잘릴 수 있으니 데이터 분포를 확인하세요.
...
* **TIP**: VRAM이 넉넉하다면 `per_device_train_batch_size=4, gradient_accumulation_steps=1` 등으로 조정해 보세요.
  (학습 스케줄도 동일 배치 크기 기준으로 다시 튜닝 필요)

<details>
<summary>🇺🇸 English Explanation</summary>

Setting `per_device_train_batch_size=2` and `gradient_accumulation_steps=2` keeps the effective batch size at 4 (2×2) per step. Reduces overhead and slightly speeds up training.
...
* **TIP**: If you have more VRAM, you can try `per_device_train_batch_size=4, gradient_accumulation_steps=1` (but then adjust your learning rate schedule accordingly).
